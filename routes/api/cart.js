...
const ensureRecipientsArray = (item) => {
  ...
  const migrated = [];
  const legacy = normalizeRecipientEntry({
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existingItemIndex > -1) {
      ...
      cart.push({
        ...
        name: product.name,
        price: Number.parseFloat(unitFinal),
...
router.put(
  ...
  asyncHandler((req, res) => {
    ...
    const fallbackIndex =
      itemIndex === -1 ? cart.findIndex((item) => item.slug === key) : itemIndex;
    ...
    if (cart[resolvedIndex]) {
      ...
      if (hasRecipientsArray) {
        ...
        item.recipients = recipients.slice(0, currentQty).map(normalizeRecipientEntry);
      }
