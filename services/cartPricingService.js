...
const normalizeSelectedAttributeValueIds = (value) => {
  ...
  const parsed = arr.map((v) => Number.parseInt(v, 10)).filter((v) => Number.isFinite(v) && v > 0);
...
const getPricingForProductSelection = async ({ slug, selectedAttributeValueIds }) => {
  ...
  const deltaGrossSum = selectedAttributes.reduce(
...
const priceCartForOrder = async (cart) => {
  ...
  const pricedItems = cart.map((item) => {
    ...
    const legacyEntry =
      (item.recipientName ?? "") || (item.dedication ?? "")
        ? { recipientName: item.recipientName, dedication: item.dedication }
        : null;
    const effectiveRecipients = recipientsRaw.length
      ? recipientsRaw
      : legacyEntry
        ? [legacyEntry]
        : [];
